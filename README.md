...
รอให้ขึ้น `MAMUDI is ready` คำสั่งเดียวนี้จะสร้าง `.venv`, ติดตั้ง dependencies, สร้าง `.env`, สร้างฐานข้อมูล SQLite, สินค้า 20 รายการ, รูปภาพ และบัญชี Admin/Customer ให้ครบ ต้องใช้อินเทอร์เน็ตตอนดาวน์โหลด dependencies ครั้งแรก
...
แต่ละเครื่องได้รหัสผ่านใหม่ของตัวเอง ไม่มีรหัสผ่าน Admin กลางที่เผยแพร่บน GitHub บัญชี Admin เริ่มต้นชื่อ `admin` ส่วน Customer ชื่อ `customer`
...
`.env`, `DEMO_ACCESS.md`, ฐานข้อมูล SQLite, `media/`, `.venv/`, `node_modules/` และ logs เป็นข้อมูลเฉพาะเครื่อง จึงถูกยกเว้นใน `.gitignore` คำสั่ง setup สร้างไฟล์ที่จำเป็นเหล่านี้ใหม่ให้หลัง clone
...
- รันซ้ำไม่เพิ่มข้อมูลซ้ำ และไม่รีเซ็ตสต็อก ชื่อสินค้า ราคา หรือรหัสผ่านเดิม
